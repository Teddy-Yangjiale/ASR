...
- If `asr.ckpt` stalls at 0%, cache the model with `make cache-speechbrain-model-direct`, then run `make validate-speechbrain-model`; use `HF_ENDPOINT=https://hf-mirror.com` if Hugging Face is unreachable from WSL.
